...
import { processManagementCommissionFromContract } from "@/utils/managementCommission";
...
export async function createIncomeOrExpenseFromInvoice(invoice) {
    try {
        // استثناء فواتير الإيجار والضرائب من نظام الفواتير
        if (invoice.invoiceType === "RENT" || invoice.invoiceType === "TAX") {
            // لا يتم تسجيل فواتير الإيجار والضرائب في نظام الفواتير
            // عمولة الإدارة يتم إنشاؤها كدفعة منفصلة عند إنشاء العقد
            return {message: "فواتير الإيجار والضرائب لا تُسجل في النظام", status: 200};
        }
...
export async function getInvioces(page, limit, searchParams) {
    ...
    try {
        ...
        if (invoiceType !== "ALL" && invoiceType) {
            console.log("DEBUG: invoiceType received:", invoiceType, "Type:", typeof invoiceType, "IsArray:", Array.isArray(invoiceType));
            
            // إذا كان invoiceType مصفوفة
            if (Array.isArray(invoiceType) && invoiceType.length > 0) {
                console.log("DEBUG: Adding array filter with 'in' operator:", invoiceType);
                conditions.push({
                    invoiceType: {
                        in: invoiceType,
                    },
