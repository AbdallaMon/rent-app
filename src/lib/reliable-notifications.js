...
async function getTeamNumbers() {
  try {
    ...
    if (!settings) {
      console.warn('⚠️ لم يتم العثور على إعدادات فريق العمل، استخدام الأرقام الافتراضية');
    ...
    return {
      ...
      PUBLIC_RELATIONS: {
        original: settings.customerServicePhone?.replace('971', '0') || '0506677779',
 ...
 */
async function sendReliableNotification(phoneNumber, message, recipientName, maxRetries = NOTIFICATION_CONFIG.MAX_RETRIES) {
  const url = `https://graph.facebook.com/v18.0/${PHONE_NUMBER_ID}/messages`;
  
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`📤 محاولة ${attempt}/${maxRetries} - إرسال لـ ${recipientName} (${phoneNumber})`);
      ...
      if (response.ok) {
        console.log(`✅ نجح إرسال لـ ${recipientName} - Message ID: ${result.messages[0].id}`);
        return {
          success: true,
        ...
        console.log(`❌ فشل المحاولة ${attempt} لـ ${recipientName}:`, result.error?.message);
        ...
        if (attempt < maxRetries) {
          console.log(`⏰ انتظار ${NOTIFICATION_CONFIG.RETRY_DELAY}ms قبل المحاولة التالية...`);
          await new Promise(resolve => setTimeout(resolve, NOTIFICATION_CONFIG.RETRY_DELAY));
        }
      }
      
    } catch (error) {
      console.log(`❌ خطأ في المحاولة ${attempt} لـ ${recipientName}:`, error.message);
...
📞 +971507935566`;
  ...
  try {    // إرسال للفني
    ...
    await new Promise(resolve => setTimeout(resolve, NOTIFICATION_CONFIG.BATCH_DELAY));
  ...
  results.forEach(result => {
    if (result.success) {
      console.log(`   ✅ ${result.recipient}: نجح في المحاولة ${result.attempt}`);
    } else {
...
📞 +971507935566`;
  try {
    ...
    console.log(`   ${result.success ? '✅' : '❌'} خدمة العملاء: ${result.success ? 'نجح' : 'فشل'}`);
      return {
    ...
    console.error('❌ خطأ عام في إرسال إشعار الشكوى:', error);    return {
      success: false,
      successRate: 0,
      successCount: 0,
      totalCount: 1,
...
function getComplaintCategoryName(type) {
  const categories = {
    'PROPERTY_ISSUE': 'مشكلة في العقار',
    'RENT_ISSUE': 'مشكلة في الإيجار',
    'NEIGHBOR_ISSUE': 'مشكلة مع الجيران',
  ...
  return categories[type] || type || 'غير محدد';
}

/**
 * اختبار نظام الموثوقية
 */
async function testReliabilitySystem() {
  console.log('🧪 اختبار نظام الإشعارات الموثوق...\n');
  
  // بيانات اختبار طلب صيانة
  const testMaintenanceData = {
    id: 'TEST_MAINT_' + Date.now(),
    type: 'plumbing',
    description: 'تسريب في حنفية المطبخ - اختبار نظام الموثوقية',
    client: { name: 'عميل الاختبار', phone: '971506111139' },
    property: { name: 'مجمع الاختبار السكني' },
    unit: { unitNumber: 'T-101' }
  };
  ...
  const testComplaintData = {
    ...
    type: 'PROPERTY_ISSUE',
    description: 'مشكلة في المصعد - اختبار نظام الموثوقية',
    client: { name: 'عميل الاختبار', phone: '971506111139' },
    property: { name: 'مجمع الاختبار السكني' },
    unit: { unitNumber: 'T-101' }
  };
  
  console.log('1️⃣ اختبار إشعارات طلب الصيانة:');
  const maintenanceResult = await sendMaintenanceNotificationsReliable(testMaintenanceData);
  
  console.log('\n2️⃣ اختبار إشعارات الشكوى:');
  const complaintResult = await sendComplaintNotificationsReliable(testComplaintData);
  
  console.log('\n' + '='.repeat(60));
  console.log('📊 تقرير الموثوقية النهائي:');
  console.log('='.repeat(60));
  console.log(`🔧 طلب الصيانة: ${maintenanceResult.successRate}% (${maintenanceResult.summary})`);
  console.log(`📝 الشكوى: ${complaintResult.successRate}% (${complaintResult.summary})`);
  
  const overallSuccess = maintenanceResult.success && complaintResult.success;
  console.log(`\n🎯 النتيجة الإجمالية: ${overallSuccess ? '✅ نجح 100%' : '⚠️ يحتاج مراجعة'}`);
  
  return { maintenanceResult, complaintResult, overallSuccess };
}
