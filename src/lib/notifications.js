export async function sendNotifications(rentAgreement, options = {}) {
    ...
    try {
        ...
        const installmentsResponse = await fetch(`${baseUrl}/api/main/installments?rentAgreementId=${rentAgreement.id}`);
        if (installmentsResponse.ok) {
            const installmentsData = await installmentsResponse.json();
            installments = installmentsData.data || [];
            console.log(`✅ Fetched ${installments.length} installments for rent agreement ${rentAgreement.id}`);
        } else {
            console.warn(`⚠️ Failed to fetch installments: ${installmentsResponse.status}`);
        }
    } catch (error) {
    ...
    if (rentAgreement.renter.email) {
        console.log(`📧 Sending ${renterLanguage === 'ar' ? 'Arabic' : 'English'} email notification to ${rentAgreement.renter.email}`);
        notifications.push(
            ...
            .then(response => {
                if (!response.ok) {
                    ...
                    console.log(`✅ ${renterLanguage === 'ar' ? 'Arabic' : 'English'} email notification sent successfully`);
