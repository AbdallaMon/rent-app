...
const ExpiringContractsReport = () => {
  ...
  const applyFilters = () => {
    ...
    if (selectedProperty !== "all") {
      filtered = filtered.filter(
        (contract) => contract.unit.property.id === selectedProperty
      );
    }
  ...
  const handleDownloadCSV = () => {
    ...
    link.setAttribute(
      "download",
      `تقرير_العقود_المنتهية_${dayjs().format("YYYY-MM-DD")}.csv`
    );
    ...
    switch (type) {
      ...
      case "month1":
        return reportData.filter(
          (c) => c.daysRemaining >= 0 && c.daysRemaining <= 30
        ).length;
      case "month2":
        return reportData.filter(
          (c) => c.daysRemaining > 30 && c.daysRemaining <= 60
        ).length;
      default:
