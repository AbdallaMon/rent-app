...
const PropertyWrapper = () => {
    ...
    async function getUnits() {
        ...
        const data = await res.json();
        const dataWithLabel = (data || []).map((item) => { // Ensure data is an array
            return {
                ...item,
                name: item.number,
    ...
    async function handleDelete(id) {
        const res = await submitData(null, null, id, "DELETE", null, null, "main/units");
        const filterData = (data || []).filter((item) => item.id !== res.id); // Ensure data is an array
        setData(filterData);
        setTotal((old) => old - 1);
    ...
    const columns = [
        ...
        },
        {
            ...
            printable: true,
            cardWidth: 48,
            renderCell: (params) => <>{formatCurrencyAED(params.row.yearlyRentPrice)}</>,
        },
        {
    ...
    async function submitRent(formData) {
        ...
        const response = await submitRentAgreement(formData, setSubmitLoading, null, null, null, true);
            ...
            >
                ...
                <FormControl sx={{ mb: 2, maxWidth: 300 }}>
                    ...
                    >
                        ...
                        {(properties || []).map((property) => ( // Ensure properties is an array
        ...
        <Modal open={openModal} onClose={() => handleClose()}>
            {canEdit ? (
                ...
                <Box sx={simpleModalStyle}>
                    <Typography variant={"h5"} align={"center"} color={"error"} sx={{ fontWeight: "bold" }}>
