...
import { AllContracts, ActiveContracts, ExpiredContracts, CancelledContracts, Home } from "@/components/ui/icons";
...
const RentWrapper = ({propperty, statusFilter, rentedFilter}) => {
    ...
    } = useDataFetcher(`main/rentAgreements?rented=expired&sort=createdAt&order=asc&`);
    ...
    const showOnlyExpired = statusFilter === 'expired' && rentedFilter === 'expired';
    const showOnlyActive = statusFilter === 'active' || (statusFilter === 'active' && rentedFilter === 'true');
    ...
    const propertyDisplayName = selectedProperty ? ` - ${selectedProperty.name}` : '';
    ...
    useEffect(() => {
        if (propperty !== undefined) { // Only trigger on actual changes, not initial load
            setRender(prev => !prev);
    ...
    async function getUnits() {
        ...
        const res = await fetch(
              "/api/fast-handler?id=unit&propertyId=" + propertyId + "&availableOnly=true",
        );
        const data = await res.json();
    ...
    const handleRenewSubmit = async (formData) => {
        if (!validateTotalPrice(formData)) {
            setSnackbarMessage("المجموع الكلي للأقساط لا يتطابق مع السعر الكلي. يرجى التحقق من المدخلات.");
          ...
          <>
              ...
              >
                  <Alert onClose={handleSnackbarClose} severity="error">
                      {snackbarMessage || "المجموع الكلي للأقساط لا يتطابق مع السعر الكلي. يرجى التحقق من المدخلات."}
                  </Alert>
              </Snackbar>
              <RenewRentModal
                    open={renewModalOpen}
