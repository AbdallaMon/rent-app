...
const RenterWrapper = () => {
    ...
    const [modifiedRenterInputs, setModifiedRenterInputs] = useState(renterInputs);
    ...
    useEffect(() => {
        ...
        const updateInputs = (language = "ARABIC") => {
            ...
            const prefersEnglishIndex = updatedInputs.findIndex(input => 
                input.data.id === "prefersEnglish"
            );
            
            if (prefersEnglishIndex !== -1) {
                updatedInputs[prefersEnglishIndex].data.defaultValue = (language === "ENGLISH");
            }
    ...
    const handleSubmit = async (formData, action) => {
        ...
        if ('prefersEnglish' in processedData) {
            processedData.language = processedData.prefersEnglish ? "ENGLISH" : "ARABIC";
    ...
    const columns = [
        ...
        },
        {
            ...
            width: 150,
            printable: true,
            valueGetter: (params) => params.row.language === "ARABIC" ? "العربية" : "الإنجليزية",
            cardWidth: 48,
        },
        {
            field: "actions",
