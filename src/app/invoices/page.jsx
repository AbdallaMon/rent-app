...
const PaymentStatusChip = ({ invoice }) => {
  ...
  if (invoice.payment) {
    ...
    const isPaid =
      invoice.payment.amount && invoice.amount <= invoice.payment.amount;
    const isOverdue =
      invoice.payment.dueDate && new Date(invoice.payment.dueDate) < new Date();
...
const InvoiceStatsCards = ({ invoices }) => {
  const stats = useMemo(() => {
    ...
    const ownerExpenses = invoices.filter(
      (inv) => inv.invoiceType === "MAINTENANCE"
    );
...
const InvoicePage = () => {
  ...
  const filteredProperties = useMemo(() => {
    ...
    return properties.filter((property) => {
      ...
      return (
        propertyOwnerId && String(propertyOwnerId) === String(selectedOwner)
      );
    });
      ...
      if (paymentStatus !== "all") {
        ...
        if (invoice.payment) {
          ...
          const isOverdue =
            ...
            new Date(invoice.payment.dueDate) < new Date();
        ...
        if (paymentStatus === "pending" && invoiceStatus !== "pending")
          return false;
        if (paymentStatus === "overdue" && invoiceStatus !== "overdue")
          return false;
      }
  ...
  useEffect(() => {
    if (selectedOwner && selectedProperty) {
      const propertyBelongsToOwner = filteredProperties.find(
  ...
  const createFilterSummary = () => {
    ...
    if (Array.isArray(invoiceType) && invoiceType.length > 0) {
      const typeNames = invoiceType.map(
        (type) => invoiceTypeMapping[type] || type
      );
