...
import { requestEditInputs, complaintInputs } from "../reqeustInputs";
...
export default function ComplaintReqeusts({ searchParams }) {
  return (
    <TableFormProvider url={"fast-handler"}>
      <ComplaintReqeustWrapper />
    </TableFormProvider>
  );
}

const ComplaintReqeustWrapper = () => {
  const {
    data,
    loading,
    page,
    ...
    filters,
    setFilters,
  } = useDataFetcher(`request/complaint`);
  ...
  const dataInputs = complaintInputs.map((input) => {
